...
for l=1:size(models,2)
    ...
    for j=1:size(shocks,2)
        ...
        y = [pi(1:end-1)*(100/max_r) A_u*u_hat(1:end-1)*(100/max_r)  A_m*mismatch_hat(1:end-1)*(100/max_r) A_omega*omega_hat(1:end-1)*(100/max_r) A_lambda*lambda_hat(1:end-1)*(100/max_r) (A_e*(lambda_hat(2:end)+W_hat(2:end))+beta*pi(2:end))*(100/max_r) (pm_hat(1:end-1)-kappa*z_hat(1:end-1))*(100/max_r)]; 
        ...
        title(name);
        legend('unemployment', 'mismatch', 'thightness','lambda', 'expectation', 'shock', 'pi');
